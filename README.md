...
* The metadata description of the OData service is used to declare all the `struct`s and `enum`s needed to interact with your chosen OData service.
...
These redundant declarations have been replcaed by the use of the types `EdmType` and `EdmPrimitive` &mdash; both of which implement `std::fmt::Display`.
